...
const buildFull = async () => {
    // rollup打包的配置信息
    const config = {
        input: path.resolve(hlRoot, 'index.ts'), // 打包的入口
...
async function buildEntry() {
    const entryFiles = await fs.readdir(hlRoot, { withFileTypes: true });
    const entryPoints = entryFiles
        .filter((f) => f.isFile())
        .filter((f) => !['package.json'].includes(f.name))
        .map((f) => path.resolve(hlRoot, f.name));
    ...
    const config = {
        ...
        external: (id: string) => /^vue/.test(id) || /^@hl-plus/.test(id),
    };
